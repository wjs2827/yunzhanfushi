...
function loadNecklineInfo(){
       ...
       $.ajax({
		...
		success:function(response){
			...
			var type=$("#spuType").val();
			if(type!=undefined&&type==1){
				initNeckSkuItemList($("#id").val());
			}
  		}
  	});
}
...
//初始化可改装SKU属性值信息列表
function initNeckSkuItemList(spuId){
   if(spuId!=undefined && spuId !=null && spuId !=''){
	   $.ajax({
		 url:contextPath+"/admin/neck/queryNeckSkuListBySpuId",
		 type:"post",
		 data:{spuId:spuId},
		 dataType:"json",
		 success:function(json){
			 //选中已经选择的规格信息
			 checkBoxNeckSkuValue(json.skuValueList);
			 //根据索引赋值给SKU信息
			 SetNeckSkuValue(json.skuList);
		 }
	   });
   }
}
...
//根据数据ID找到对于的复选框让其选中
function  checkBoxNeckSkuValue(list){
	  for(var i=0;i<list.length;i++){
     	$("input[name="+list[i].key+"]").attr({"checked":"checked"});
	  }
	  step.Creat_Table();
}

//根据数据ID找到对于的复选框让其选中
function  SetNeckSkuValue(skuList){
	  for(var i=0;i<skuList.length;i++){
		  $("#neckimg-key"+i).attr({"src":$("#qiniupath").val()+skuList[i].picKey});
		  $("#neckimg-keys"+i).val(skuList[i].picKey);
		  $("#neck_price"+i).val(skuList[i].salePrice);
	  }
}
...
function initSkuList(id){
   ...
   if(spuId==undefined || spuId ==null || spuId =='')
	   spuId="";
	var type=$("#isUnifiedSpecs").val();
	//多规格数据加载
    $.ajax({
	 url:contextPath+"/admin/sku/list",
	 type:"get",
	 data:{classId:classId,id:spuId},
	 dataType:"html",
	 success:function(response){
...
function TYskuItem(spuId){
	   //统一规格加载
	   $.ajax({
			 url:contextPath+"/admin/good/queryGoodSkuBySpuId",
			 type:"post",
			 data:{spuId:spuId},
			 dataType:"json",
			 success:function(json){
				 $("#skuCode").val(json.sku.skuCode);
				 $("#sku_price").val(json.sku.price);
				 $("#sku_market").val(json.sku.marketPrice);
				 $("#sku_stock").val(Number(json.sku.stockCount)-(Number(json.sku.lockCount)+Number(json.sku.saleCount)));
			 },error:function(data){
				 console.log("系统异常");
			 }
		    });
...
//初始化SKU属性值信息列表
function initGoodSkuItemList(spuId){
   if(spuId!=undefined && spuId !=null && spuId !=''){
	   $.ajax({
		 url:contextPath+"/admin/good/queryGoodSkuListBySpuId",
		 type:"post",
		 data:{spuId:spuId},
		 dataType:"json",
		 success:function(json){
			 //选中已经选择的规格信息
			 checkBoxGoodSkuValue(json.skuValueList);
			 //根据索引赋值给SKU信息
			 SetGoodSkuValue(json.skuList);
		 }
	   });
   }
}
...
//根据数据ID找到对于的复选框让其选中
function  checkBoxGoodSkuValue(list){
	  for(var i=0;i<list.length;i++){
			$("input[name="+list[i].key+"]").attr({"checked":"checked"});
	  }
	  sku.Creat_Table();
...
//根据数据ID找到对于的复选框让其选中
function  SetGoodSkuValue(skuList){
	  for(var i=0;i<skuList.length;i++){
		  $("#skuImg-key"+i).attr({"src":$("#qiniupath").val()+skuList[i].picKey});
		  $("#skuImg-keys"+i).val(skuList[i].picKey);
		  $("#sku_price"+i).val(skuList[i].price);
		  $("#sku_market"+i).val(skuList[i].marketPrice);
		  $("#sku_stock"+i).val(skuList[i].stockCount);
		  $("#sku_stock"+i).val(Number(skuList[i].stockCount)-(Number(skuList[i].lockCount)+Number(skuList[i].saleCount)));
	  }
}

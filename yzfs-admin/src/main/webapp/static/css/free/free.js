...
var newTR="<tr >"+
		    "<td>选择可配送区域<a data-toggle='modal' href='#freeDialog'><img src='/yzfs-admin/static/image/icon/fedit.png' style='width: 20px; height: 20px;'/></a><img class='delImage'  src='/yzfs-admin/static/image/icon/fdel.png' style='width: 20px; height: 20px;'/><br/>" +
	 ...
	 $.ajax({
		 ...
		 success:function(data){
			 ...
			 if(cidlength==data.list.length){
			    $(".cities").append("<li> <label class='checkbox'><img src='/yzfs-admin/static/image/zhong.png' onclick='checkAll();' name='checkCityItemImg'   class='checkAllImg'><span>全选</span> </label></li>");
			 }else{
			    $(".cities").append("<li> <label class='checkbox'><img src='/yzfs-admin/static/image/kong.png' onclick='checkAll();'  name='checkCityItemImg'  class='checkAllImg'><span>全选</span> </label></li>");
			 }
			 $.each(data.list,function(key,val){
				 var selectedCID=$("#selectedCID"+val.id).val();
				 if(selectedCID==val.id){
					 $(".cities").append("<li> <label class='checkbox'><img src='/yzfs-admin/static/image/zhong.png'  name='checkCityItemImg' onclick='checkItem("+key+");' class='checkImg"+key+"'><input type='hidden' class='js_areas_checkitem' id='checkImg"+key+"' name='checkCname"+val.id+"'  data-name='"+val.name+"' data-myid='"+val.id+"' type='text'><span>"+val.name+"</span> </label></li>");
				 }else{
					 $(".cities").append("<li> <label class='checkbox'><img src='/yzfs-admin/static/image/kong.png'  name='checkCityItemImg' onclick='checkItem("+key+");' class='checkImg"+key+"'><input type='hidden' class='js_areas_checkitem' id='checkImg"+key+"'  name='checkCname"+val.id+"' data-name='"+val.name+"' data-myid='"+val.id+"' type='text'><span>"+val.name+"</span> </label></li>");
				 }
